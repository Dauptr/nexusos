...
// PUT - Update an existing image (for editing)
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, prompt, imageUrl, style } = body;

    if (!id || !imageUrl) {
      return NextResponse.json(
        { error: 'Image ID and imageUrl are required' },
        { status: 400 }
      );
    }
    ...
    const image = await db.generatedImage.update({
      where: { id },
      data: {
        prompt: prompt || existingImage.prompt,
        imageUrl,
        style: style || existingImage.style,
      },
    });
