...
type NavSection = 'home' | 'create' | 'database' | 'chat' | 'youtube' | 'tiktok' | 'tetris' | 'spotify' | 'notifications' | 'profile' | 'p2p' | 'invite' | 'admin' | 'turbo' | 'about' | 'nexus-ai' | 'soul' | 'games' | 'memories' | 'family' | 'create-space'
...
function AdminPage({ features, setFeatures }: { features: FeatureFlags; setFeatures: (f: FeatureFlags) => void }) {
  ...
  const [loading, setLoading] = useState(true)
  const [activeTab, setActiveTab] = useState<'stats' | 'users' | 'features' | 'chat-console' | 'music' | 'resets' | 'urls' | 'claude' | 'family' | 'backup'>('stats')
          ...
          <div className="flex gap-2 mb-3 flex-wrap">
            {voices.map(voice => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs transition ${ttsVoice === voice.id ? 'bg-cyan-500 text-white' : 'bg-white/10 text-white/60 hover:bg-white/20'}`}
              >
                {voice.name}
              </button>
            ))}
