...
type NavSection = 'home' | 'create' | 'database' | 'chat' | 'youtube' | 'tiktok' | 'tetris' | 'spotify' | 'notifications' | 'profile' | 'p2p' | 'invite' | 'admin' | 'turbo' | 'about' | 'nexus-ai' | 'soul' | 'games' | 'memories' | 'family'
        ...
        <nav className="flex flex-col gap-1 flex-1 overflow-y-auto overflow-x-hidden pr-1 min-h-0">
          ...
          {user ? (
            <>
              ...
              {features.p2pChat && <NavItem active={nav === 'p2p'} onClick={() => { setNav('p2p'); setSidebarOpen(false) }} icon={<P2PIcon />} collapsed={sidebarCollapsed}>Messages</NavItem>}
              {features.notifications && <NavItem active={nav === 'notifications'} onClick={() => { setNav('notifications'); setSidebarOpen(false) }} icon={<NotificationIcon />} collapsed={sidebarCollapsed}>Notifications</NavItem>}
...
function AdminPage({ features, setFeatures }: { features: FeatureFlags; setFeatures: (f: FeatureFlags) => void }) {
  ...
  const [loading, setLoading] = useState(true)
  const [activeTab, setActiveTab] = useState<'stats' | 'users' | 'features' | 'chat-console' | 'music' | 'resets' | 'urls' | 'claude' | 'family'>('stats')
