...
export async function GET(request: NextRequest) {
  try {
    ...
    if (tab === 'app-urls') {
      const urlsSetting = await prisma.appSettings.findUnique({
    ...
    const [
      ...
      prisma.passwordReset.count({ where: { approved: false } })
    ])
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'delete' && userId) {
      ...
      await prisma.p2PMessage.deleteMany({ where: { fromUserId: userId } })
      await prisma.p2PMessage.deleteMany({ where: { toUserId: userId } })
      await prisma.chatMessage.deleteMany({ where: { userId } })
      await prisma.generatedImage.deleteMany({ where: { userId } })
      await prisma.inviteLink.deleteMany({ where: { createdById: userId } })
    ...
    if (action === 'rejectReset' && resetId) {
      await prisma.passwordReset.delete({ where: { id: resetId } })
      return NextResponse.json({ success: true })
    }
