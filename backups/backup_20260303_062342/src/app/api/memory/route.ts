...
export async function GET(request: NextRequest) {
  ...
  try {
    if (category && key) {
      // Get specific memory
      const memory = await prisma.claudeMemory.findUnique({
        where: { category_key: { category, key } }
      })
      if (memory) {
        // Update access stats
        await prisma.claudeMemory.update({
          where: { id: memory.id },
          data: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const memory = await prisma.claudeMemory.upsert({
      where: { category_key: { category, key } },
      create: {
        category,
        key,
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await prisma.claudeMemory.delete({
      where: { category_key: { category, key } }
    })
